...
public class StudentDAO extends DAO{
	...
	public int insertStd(Student std) {
		...
		try {
			conn();
			String sql = "INSERT INTO student VALUES (?,?,?,?)"; //물음표 순서는 테이블 구조따라서
			pstmt = conn.prepareStatement(sql);
	...
	public List<Student> getAnalyze(){
		...
		try {
			conn();
			String sql = "SELECT std_major, sum(std_point) total, avg(std_point) from student group by std_major";
			pstmt = conn.prepareStatement(sql);
			...
			while(rs.next()) {
				std = new Student();
				std.setStdMajor(rs.getString("std_major"));
				std.setSum(rs.getDouble("total")); //별칭 사용
